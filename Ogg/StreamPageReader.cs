...
{
    class StreamPageReader
    {
        private readonly PageReader _reader;
        public readonly List<long> _pageOffsets = new List<long>();
        private int _lastSeqNbr, _lastPageIndex = -1, _lastPagePacketCount, _lastPageOverhead;
        private int? _firstDataPageIndex;
        private long _maxGranulePos, _lastPageGranulePos;
        private bool _lastPageIsResync, _lastPageIsContinuation, _lastPageIsContinued;
        private Memory<byte>[] _cachedPagePackets;
        public PacketProvider PacketProvider;
        public StreamPageReader(PageReader pageReader, int streamSerial)
        {
            _reader = pageReader;
        ...
        public int FindPageForward(int pageIndex, long pageGranulePos, long granulePos)
        {
